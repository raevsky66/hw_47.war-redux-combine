...
import {changePageAction, setCompWinsAction, setPlayerWinsAction, setWinAction} from "../redux/gameActions";
...
const Game = () =>
{
    ...
    let win = useRef({playerWins: 0,compWins:0})
    ...
    const handleClickNext = () =>
    {
        if (compCards.current.length)
        {
            ...
            if (compCard.value > playerCard.value)
                win.current.compWins++;
            if (compCard.value < playerCard.value)
                win.current.playerWins++;
        ...
        else
        {
            ...
            console.log(win.current.playerWins+" -  "+win.current.compWins)
            dispatch(setWinAction(win.current))
